...
#         print("ok_scene_tokens after filtering: ", len(ok_scene_tokens))
    ...
    def __getitem__(self, index):
        ...
        # mask = np.logical_and(mask, points[1, :] <= h_og)
        # print(f"After y <= h_og: {np.sum(mask)}")
...
class NuscenesImageLidarDataset_with_path(Dataset):
    ...
    def __len__(self):
        return len(self._frames) * len(self.NUSCENES_CAM_NAMES)

    def __getitem__(self, index):
        # Index to token, get relevant data and tokens
        sample_token = self._frames[index // len(self.NUSCENES_CAM_NAMES)]
        cam_name = self.NUSCENES_CAM_NAMES[index % len(self.NUSCENES_CAM_NAMES)]
        ...
        # Load image
        im_path = osp.join(self._nusc.dataroot, cam["filename"])
        im = Image.open(osp.join(self._nusc.dataroot, cam["filename"]))
        ...
        return im, points_cam.contiguous(), pcl_path
        # return im, points_cam.contiguous(), im_path
...
def build_loader(
    ...
    elif dataset_name == "with_path":
        dataset = NuscenesImageLidarDataset_with_path(datadir, img_transform=clip_preprocess, split=split)
        loader = DataLoader(
            dataset,
            num_workers=num_workers,
            batch_size=batch_size,
            collate_fn=_collate_fn_path,
            pin_memory=False,
            shuffle=shuffle,
        )
        return loader
